...
CMTPursue::CMTPursue(CRCGEnemy* npc, CState* nextState) :
...
void CMTPursue::Update()
{
	...
	if (CanAttackPlayer() && m_NPC->GetCurrentState() == (int)EEnemyState::NORMAL)
	{
		...
		if (prob < 0.5f && player->CheckState(EKyokoState::ATTACK))
		{
			...
			m_NextState = (CState*)(new CMTBlock(m_NPC, m_NextState));
			m_Stage = EVENT::EXIT;
			return;
		}
		...
		if (prob <= 0.2f)
		{
			SAFE_DELETE(m_NextState);
			m_NPC->GetNavAgent()->SetInterrupted(true);
			m_NextState = (CState*)(new CMTAttack(m_NPC, m_NextState));
			m_Stage = EVENT::EXIT;
			return;
		}
	}
...
bool CMTPursue::CanAttackPlayer()
{
