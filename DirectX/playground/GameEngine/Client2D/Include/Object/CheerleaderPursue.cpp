...
CCheerleaderPursue::CCheerleaderPursue(CRCGEnemy* npc, CState* nextState) :
...
CCheerleaderPursue::~CCheerleaderPursue()
{
}

void CCheerleaderPursue::Update()
{
	...
	if (CanAttackPlayer() && m_NPC->GetCurrentState() == (int)EEnemyState::NORMAL)
	{
		...
		if (prob < 0.5f && player->CheckState(EKyokoState::ATTACK))
		{
			...
			m_NextState = (CState*)(new CCheerleaderBlock(m_NPC, m_NextState));
			m_Stage = EVENT::EXIT;
			return;
		}
		...
		if (prob <= 0.2f)
		{
			SAFE_DELETE(m_NextState);
			m_NPC->GetNavAgent()->SetInterrupted(true);
			m_NextState = (CState*)(new CCheerleaderAttack(m_NPC, m_NextState));
			m_Stage = EVENT::EXIT;
			return;
		}
	}
...
bool CCheerleaderPursue::CanAttackPlayer()
{
