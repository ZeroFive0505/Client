...
CRCGStage03::CRCGStage03()
{
	SetTypeID<CRCGStage03>();
}
...
bool CRCGStage03::Init()
{
	...
	m_TileMap->Load("Stage3.gobj", SCENE_PATH);
	...
	m_Player->SetWorldPos(186.0f, (float)m_TileMap->GetTileMap()->GetBackMaterial()->GetTextureHeight() * 2.5f - 910.0f, 0.0f);
	...
	door->SetWorldPos(1265.0f, (float)m_TileMap->GetTileMap()->GetBackMaterial()->GetTextureHeight() * 2.5f - 865.0f, 0.0f);
	...
	CMT* mt = m_Scene->CreateGameObject<CMT>("Enemy 1");

	mt->SetIdleState();

	mt->SetWorldPos(700.0f, (float)m_TileMap->GetTileMap()->GetBackMaterial()->GetTextureHeight() * 2.5f - 700.0f, 0.0f);

	CClientManager::GetInst()->GetManager()->AddEnemy(mt);

	mt = m_Scene->CreateGameObject<CMT>("Enemy 2");

	mt->SetIdleState();

	mt->SetFacingDirection(false);

	mt->SetWorldPos(800.0f, (float)m_TileMap->GetTileMap()->GetBackMaterial()->GetTextureHeight() * 2.5f - 800.0f, 0.0f);

	CClientManager::GetInst()->GetManager()->AddEnemy(mt);

	mt = m_Scene->CreateGameObject<CMT>("Enemy 3");

	mt->SetIdleState();

	mt->SetFacingDirection(false);

	mt->SetWorldPos(800.0f, (float)m_TileMap->GetTileMap()->GetBackMaterial()->GetTextureHeight() * 2.5f - 700.0f, 0.0f);

	CClientManager::GetInst()->GetManager()->AddEnemy(mt);

	mt = m_Scene->CreateGameObject<CMT>("Enemy 4");

	mt->SetIdleState();

	mt->SetWorldPos(1000.0f, (float)m_TileMap->GetTileMap()->GetBackMaterial()->GetTextureHeight() * 2.5f - 900.0f, 0.0f);

	CClientManager::GetInst()->GetManager()->AddEnemy(mt);

	mt = m_Scene->CreateGameObject<CMT>("Enemy 5");

	mt->SetIdleState();

	mt->SetWorldPos(1000.0f, (float)m_TileMap->GetTileMap()->GetBackMaterial()->GetTextureHeight() * 2.5f - -850.0f, 0.0f);
	...
	m_RCGMainWidget->SetSceneChangeCallback<CRCGStage03>(this, &CRCGStage03::SceneChange);

	m_Player->SetHP();

	m_Player->SetSP();

	CInput::GetInst()->SetCallback<CRCGStage03>("Enter", KeyState_Down, this, &CRCGStage03::ReloadScene);

	return true;
}

void CRCGStage03::Start()
{
...
void CRCGStage03::CreateMaterial()
{
...
void CRCGStage03::CreateParticle()
{
...
void CRCGStage03::ReloadScene(float deltaTime)
{
