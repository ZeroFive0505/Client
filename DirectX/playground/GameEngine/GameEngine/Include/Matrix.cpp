...
{
    m = XMMatrixScaling(_v.x, _v.y, 1.0f);
}

void Matrix::Scaling(float x, float y)
{
    m = XMMatrixScaling(x, y, 1.0f);
}

void Matrix::Rotation(const Vector3& _v)
{
    Vector3 converRot = _v.ConvertAngle();

    XMVECTOR qut = XMQuaternionRotationRollPitchYaw(converRot.x, converRot.y, converRot.z);
...
{
    m = XMMatrixTranslation(_v.x, _v.y, 0.0f);
}

void Matrix::Translation(float x, float y)
{
    m = XMMatrixTranslation(x, y, 0.0f);
}
