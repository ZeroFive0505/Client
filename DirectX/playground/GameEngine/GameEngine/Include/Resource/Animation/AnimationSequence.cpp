...
#ifdef UNICODE
    WideCharToMultiByte(CP_ACP, 0, pFileName, -1, strFileName, lstrlen(pFileName), nullptr, nullptr);
#else
...
{
    const sPathInfo* pPath = CPathManager::GetInst()->FindPath(strPathName);
...
bool CAnimationSequence::SaveFullPath(const TCHAR* pFullPath)
{
    ...
    WideCharToMultiByte(CP_ACP, 0, pFullPath, -1, strFullPath, lstrlen(pFullPath), nullptr, nullptr);
...
bool CAnimationSequence::LoadFullPathMultibyte(const char* pFullPath)
{
    ...
    for (size_t i = 0; i < iCount; i++)
    {
        ...
        for (size_t j = 0; j < iBoneFrameCount; j++)
        {
            ...
            if (j < m_FrameLength)
            {
                ...
                tFrame.vScale = Vector4(pKeyFrame->vScale.x, pKeyFrame->vScale.y, pKeyFrame->vScale.z, 1.0f);
                tFrame.vTranslate = Vector4(pKeyFrame->vPos.x, pKeyFrame->vPos.y, pKeyFrame->vPos.z, 1.0f);
    ...
    m_KeyFrameBuffer->UpdateBuffer(&vecFrameTrans[0], vecFrameTrans.size());
...
void CAnimationSequence::SetPlayTime(float fTime)
{
    ...
    // 키 프레임 수 만큼 반복하며 각각의 프레임을 보간할 행렬 정볼르 위치, 크기 회전정보로 뽑아온다.
    size_t size = m_vecKeyFrame.size();
    ...
    for (size_t i = 0; i < size; i++)
    {
        ...
        size_t boneKeyFrameSize = pBoneKeyFrame->vecKeyFrame.size();

        for (size_t j = 0; j < boneKeyFrameSize; j++)
        {
...
bool CAnimationSequence::CreateSequence(bool bLoop, _tagFbxAnimationClip* pClip)
{
    ...
    size_t size = pClip->vecBoneKeyFrame.size();
    for (size_t i = 0; i < size; i++)
    {
        BoneKeyFrame* pBoneKeyFrame = new BoneKeyFrame;
        ...
        size_t boneKeyFrameSize = pClip->vecBoneKeyFrame[i].vecKeyFrame.size();
        ...
        for (size_t j = 0; j < boneKeyFrameSize; j++)
        {
            ...
            pKeyFrame->vPos = Vector3((float)vPos.mData[0], (float)vPos.mData[1],
                (float)vPos.mData[2]);
            pKeyFrame->vRot = Vector4((float)qRot.mData[0], (float)qRot.mData[1], (float)qRot.mData[2],
                (float)qRot.mData[3]);
...
#ifdef UNICODE
    WideCharToMultiByte(CP_UTF8, 0, pFullPath, -1, strFullPath, lstrlen(pFullPath), nullptr, nullptr);
...
void CAnimationSequence::ResetChageShader()
{
