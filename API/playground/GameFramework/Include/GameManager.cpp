...
CGameManager::~CGameManager()
{
	...
	// 텍스처와 동일한 방법으로 디바이스 컨텍스트를 교체하고 제거한다.
...
{
	// DeltaTime을 현재 시간 비율을 곱해서 반환한다.
	// 이렇게하면 게임을 정지하거나 계속하는것을 비율을 수정하여 가능하게 할 수 있다.
...
bool CGameManager::Init(HINSTANCE hInst)
{
	...
	// 장면관리자의 인스턴스를 가져와서 장면을 만든다.
	CSceneManager::GetInst()->CreateScene<CStartScene>();

	// 타이머를 생성한다.
	m_Timer = new CTimer;

	// 백버퍼를 생성한다. 텍스쳐와 동일한 방법
...
void CGameManager::Logic()
{
	...
	// 씬매니저에서 다음 씬으로 전환이 이루어졌을 경우
	// 참이 반한되며 따라서 게임 로직을 처음부터 다시 실행한다.
	if (Update(DeltaTime))
		return;
...
{
	// 게임 매니저는 매 프레임마다 업데이트 될때 장면관리자를 업데이트한다.
	// 이는 장면관리자에 모든 오브젝트나 리소스들이 속해있기에 그렇다.
	return CSceneManager::GetInst()->Update(DeltaTime);
}

bool CGameManager::PostUpdate(float DeltaTime)
{
	// PostUpdate도 Update와 같은 이유
