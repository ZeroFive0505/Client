...
void CEditorScene::MouseRButton(float DeltaTime)
{
	...
	{
	case ETileEditMode::Property:
		m_TileMap->ChangeTileProperty(MousePos, ETileProperty::Normal);
		break;
	case ETileEditMode::Image:
...
void CEditorScene::TileImageClear()
{
	if (!m_TileMap)
		return;

	m_TileMap->TileImageClear();
}
