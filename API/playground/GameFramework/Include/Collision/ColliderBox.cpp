...
void CColliderBox::PostUpdate(float DeltaTime)
{
	...
	m_Info.Left = Pos.x - m_Width / 2.0f + m_Offset.x;
	m_Info.Top = Pos.y - m_Height / 2.0f + m_Offset.y;
	m_Info.Right = Pos.x + m_Width / 2.0f + m_Offset.x;
	m_Info.Bottom = Pos.y + m_Height / 2.0f + m_Offset.y;
}
...
bool CColliderBox::Collision(CCollider* Dest)
{
	...
	case ECollider_Type::Sphere:
		return CCollision::CollisionBoxToSphere2(this, (CColliderSphere*)Dest);
