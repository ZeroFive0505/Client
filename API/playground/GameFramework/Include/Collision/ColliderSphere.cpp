...
void CColliderSphere::Render(HDC hDC)
{
	...
	// 먼저 정점을 옮긴다.
	MoveToEx(hDC, (int)(RenderInfo.Center.x + m_Info.Radius), (int)RenderInfo.Center.y, nullptr);

	for (int i = 0; i < 12; i++)
	{
		float Radian = DegreeToRadian((i + 1) * 360.0f / 12.0f);
...
bool CColliderSphere::Collision(CCollider* Dest)
{
	...
	case ECollider_Type::Box:
		return CCollision::CollisionBoxToSphere2((CColliderBox*)Dest, this);
